...
let bridgeIcon = L.icon({
   ...
   L.marker([51.5, -0.09], {icon: brigdeIcon}).addTo(map)

})
   let BridgeIcon = new BridgeIcon({iconUrl: 'bridge-green.png'}),
   redIcon = new BridgeIcon({iconUrl: 'bridge-red.png'}),
   orangeIcon = new BridgeIcon({iconUrl: 'bridge-orange.png'}),
   yellowIcon = new BridgeIcon({iconUrl: 'bridge-yellow.png'}),
   blackIcon = new BridgeIcon({iconUrl: 'bridge-black.png'})
    ...
    bridgeData.forEach(function (bridge) {
        ...
        bridgeMarker.bindPopup`<b>${bridge.name}</b><br>${bridge.cityState}<br>Span: ${bridge.span} meters`
      })